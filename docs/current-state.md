...
- **Progress (prior):** Executed the two Phase 6 steps that required a running app; both committed (`e721dd8`, `57836a5`).
  ...
  - **Known gaps found, NOT fixed (documented in eval-report.md as follow-up candidates)**: no per-field inline validation on `MemoryFormPage.tsx`/`LostNewPage.tsx` (both use one combined banner instead of attaching errors to the specific field, affects 2 failing eval cases); `LostRequestRequest.years` is unvalidated free text (no way to reject an impossible/backwards range, the failing eval case's original scenario doesn't even match the real schema); no `returnTo` redirect-preservation after being bounced to `/login`; `eval-error-clarity-register-weak-password` (40, fail) may be a capture-methodology false negative, needs a re-run with a longer settle before trusting the score either way.
...
**Gate G6 is now fully closed** (recordings + vision-verify done in a prior session; eval suite now 14/14 pass, baseline ratcheted, this session). Next: **Phase 7 — Global review, docs, release**:
1. Run the `review-gate` workflow once over the WHOLE codebase (`baseRef` = first commit), security emphasis: authz matrix per route × role, OWASP top-10 sweep, secrets scan, `npm audit`, error-surface audit. Fix confirmed findings, re-run battery.
2. `node scripts/check-trajectory.mjs --release` then the `trajectory-eval` workflow over all archived slices; review `docs/qa/trajectory-eval-report.md`.
3. Write `docs/technical/` (architecture, data model, auth/access, workflows, APIs/actions, integrations, operations, testing), `docs/estimation.md`, and a stakeholder delivery report with an effort log from git history.
4. Deploy (needs user-provided env/secrets and approval for the push/deploy target — ask before any deploy or push), then smoke-check the live URL and record deployment facts here.
Gate G7 → commit + push (user-approved target).
