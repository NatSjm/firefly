...
- **Date and time:** 2026-07-08 05:55:00 (UTC+02:00)
- **Current phase:** **Phase 6 — QA proof pack: recordings + eval-suite executed, real findings surfaced**
- **Active change:** none
- **Progress:** Executed the two Phase 6 steps that required a running app.
  ...
  - **Eval suite**: all 14 `evals/cases/*.eval.ts` `produce()` functions were unimplemented placeholders (literally description strings, not real output). Drove the real app (Playwright + direct API calls) to capture genuine output, graded by 5 fresh `eval-judge` agent batches (one per domain file). Result: **8/14 pass**. `docs/qa/eval-report.md` + `evals/results/latest.json` + `evals/results/manifest.json` written; baseline established via `check-eval-ratchet.mjs --update` (error-clarity 46.5, empty-state-usability 83, copy-tone 78, auth-security 86).
  - **Real bugs found and fixed this session**: (1) mobile header overlap (above); (2) disabled warmth/like button had no tooltip explaining why for anonymous users — added one; (3) report-reason textarea had no encouraging placeholder — added one; (4) memory title >255 chars returned an English (not Ukrainian) validation detail that the frontend never surfaced anyway — fixed `MemoryDtos.kt` + `GlobalExceptionHandler.kt` (Ukrainian `@Size` message + single-violation surfacing), **but this needs a backend restart to take effect and has not been re-verified**.
  - **Known gaps found, NOT fixed (documented in eval-report.md as follow-up candidates)**: no per-field inline validation on `MemoryFormPage.tsx`/`LostNewPage.tsx` (both use one combined banner instead of attaching errors to the specific field); `LostRequestRequest.years` is unvalidated free text (no way to reject an impossible/backwards range); no `returnTo` redirect-preservation after being bounced to `/login`.
  ...
  - **New known gap, NOT fixed**: the Playwright E2E suite (`e2e/*.spec.ts`, a Phase 5 deliverable) has never been run successfully against a live app — 8/14 tests fail once axios was fixed and it could actually run. Root causes: tests use `input[name="email"]` etc. selectors but the real form components never set a `name` attribute; several assertions have Playwright strict-mode violations (ambiguous text locators matching 2-3 elements, e.g. the wordmark appearing in both header and footer). A follow-up task was flagged for this (`task_135bb3c5` in this session, or re-derive from `docs/qa/eval-report.md` context if stale).
  - `npm run qa:verify` was run and regenerated `docs/qa/automated-verification-latest.md`; it currently reports the E2E suite failure above as its only red item (traceability/trajectory warnings are pre-existing and unrelated).
  - **Not yet done**: commit this work (pending user confirmation — substantial changes across FE, BE, scripts, and docs); backend restart + re-verification of the title-too-long eval fix (pending user restarting their IntelliJ run config, since the backend process wasn't started by this session).
