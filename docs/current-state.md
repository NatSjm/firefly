...
- **Progress:** Executed the two Phase 6 steps that required a running app; both committed (`e721dd8`, `57836a5`).
  ...
  - **Known gap, NOT fixed, follow-up spawned**: the Playwright E2E suite (`e2e/*.spec.ts`, a Phase 5 deliverable) has never been run successfully against a live app — 8/14 tests fail once axios was fixed and it could actually run (selectors reference nonexistent `name` attributes; several strict-mode locator ambiguities). Flagged as task `task_135bb3c5`, which the user has since started running in a separate session — check its outcome before re-deriving the same fix.
